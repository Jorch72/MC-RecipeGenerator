 ...
 * 
 * It is distributed under the terms of my Open Source License. It grants rights to read, modify, compile or run the code. It does *NOT* grant the right to redistribute this software or its modifications in any form, binary or source, except if
 * expressively granted by the copyright holder. */
...
public class ARG
{
    ...
    public static Logger argLog = LogManager.getLogger("ARG");
    ...
    {
        argLog.info("Starting " + NAME + " v" + VERSION);
    ...
    public void createRecipeImages(TextureStitchEvent.Post evt)
    {
        ...
        if (mapLoaded[0] > 0 && mapLoaded[0] == mapLoaded[1])
        {
            ...
            for (Object orecipe : CraftingManager.getInstance().getRecipeList())
            {
                ...
                if (identifier != null)
                    subFolder = GetValidFileName(identifier.modId);
    ...
    private UniqueIdentifier getUniqueIdentifier(ItemStack itemStack)
    {
        ...
        {
            Block block = ((ItemBlock) itemStack.getItem()).blockInstance;
